/* spec.js — The instruction block to keep in Claude project knowledge.

   This string is the source of truth: the "Copy format spec" button hands out
   exactly this, and SHOTLIST.md is generated from it, so the button and the
   repo can never disagree. If the grammar in parse.js changes, change this in
   the same commit and regenerate:

     node -e "import('./assets/js/studio/spec.js') \
       .then(m=>require('fs').writeFileSync('SHOTLIST.md',m.SPEC))"  */

export const SPEC=`# Palm Static — shotlist output format
